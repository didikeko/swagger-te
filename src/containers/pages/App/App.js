...
export default class App extends React.Component {
  ...
  updateEditor = () => {
    this.state.value_editor_changed = this.state.codes
      .split("\n")
      .filter(Boolean);
  ...
  save = (newValue) => {
    if (this.state.codes != null) {
      this.updateEditor();
      setTimeout(() => this.reload(newValue), 1000);
    }
    // this.updateCodesEditor(newValue);
  };

  componentDidMount() {
    fetch("http://localhost:5000/readfile", {
      method: "GET",
      mode: "cors",
      headers: {
        "Access-Control-Allow-Origin": "*",
      ...
      .then((response) => response.json())
      .then((json) => this.setState({ codes: json.data.join("\n") }))
      .catch((error) => console.log("error", error));
  }
  coba = () => {
    if (this.state.value_editor != null) {
      this.state.value_editor_changed = this.state.value_editor
        .split("\n")
        .filter(Boolean);
